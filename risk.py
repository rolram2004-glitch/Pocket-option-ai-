...
def check_demo_trade(
    ...
    balance = (
        store.strategy_ledger_balance(profile.chat_id, strategy_variant)
        if strategy_variant is not None
        else profile.demo_balance
    )
    if profile.trade_amount > balance:
        return False, "Saldo DEMO insufficiente."
    count, pnl = store.daily_stats(profile.chat_id, strategy_variant)
